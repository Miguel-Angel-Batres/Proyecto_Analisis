...
module.exports = () => {
  ...
  router.post("/servicios", async (req, res) => {
    try {
      const { nombre, descripcion, precio, duracion, categoria, imagen } =
        req.body;
      const servicioData = {
        nombre,
        descripcion,
      ...
      res
        .status(201)
        .json({ message: "Servicio agregado exitosamente", id: docRef.id });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });
  ...
  router.post("/uñas", async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "Estilo agregado exitosamente", id: docRef.id });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });
  ...
  router.get("/uñas", async (req, res) => {
    try {
      ...
      const estilos = estilosSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
  ...
  router.post("/peinados", async (req, res) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "Peinado agregado exitosamente", id: docRef.id });
    } catch (error) {
      res.status(500).json({ error: error.message });
    }
  });
  ...
  router.get("/peinados", async (req, res) => {
    try {
      ...
      const peinados = peinadosSnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
